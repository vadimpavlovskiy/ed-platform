...
const ControllerMenu = () => {
    ...
    return (
        <div className="controller-menu">
            ...
            <div className="controller-items">{items.map((item, index)=> {return <DropdownItem onClick={()=>dispatch(setStatus(item))} id={item.id} isSelected={item.isSelected} key={index} title={item.title} />})}</div>
        </div>
    )
}
